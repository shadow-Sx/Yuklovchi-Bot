...
def get_channel_id_forward(message):
    ...
    if message.forward_from_chat:
        ...
        bot.reply_to(message, f"📢 Kanal: {chat.title}\n🆔 ID: {chat.id}")
    else:
...
def get_group_id_forward(message):
    ...
    if message.forward_from_chat:
        chat = message.forward_from_chat
        bot.reply_to(message, f"👥 Guruh: {chat.title}\n🆔 ID: {chat.id}")
    else:
...
def get_bot_id(message):
    ...
    try:
        ...
        bot.reply_to(message, f"🤖 Bot: @{text}\n🆔 ID: {chat.id}")
    except:
        bot.reply_to(message, "❌ Bot topilmadi!")
    admin_state[uid] = {}
...
def get_user_id(message):
    ...
    if message.forward_from:
        ...
        bot.reply_to(message, f"👤 Foydalanuvchi: {user.first_name}\n🆔 ID: {user.id}")
    else:
...
def multi_mode_select(call):
    ...
    if call.data == "multi_mode_single":
        ...
        bot.edit_message_text("📥 1-1 rejimi: Kontentlarni tashlang. Tugagach /stop yozing.", call.message.chat.id, call.message.message_id)
    else:
        admin_state[uid] = "multi_add_batch"
        admin_data[uid] = {"batch": []}
        bot.edit_message_text("📥 ♾️-1 rejimi: Barcha kontentlarni yuboring, tugagach /stop bosing.", call.message.chat.id, call.message.message_id)
...
def stop_command(message):
    ...
    if state == "multi_add_batch":
        ...
        docs = []
        for item in batch:
            docs.append({"type": item["type"], "file_id": item.get("file_id"), "caption": item.get("caption"), "text": item.get("text"), "code": code, "order": item["order"]})
...
# ==========================
#   /start (ENG SODDA - ISHLASHI KAFOLATLANGAN)
# ==========================
